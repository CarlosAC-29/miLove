...
export const financeRepository = {
  async listTransactions(userId: string, context: FinanceContext) {
    const result = await db.query(
      `select id, amount, type, category, description, date, context, owner_id as "ownerId", created_at as "createdAt"
       from transactions
  ...
  async createTransaction(input: {
    ...
    const result = await db.query(
      `insert into transactions (amount, type, category, description, date, context, owner_id, user_id)
       values ($1, $2, $3, $4, $5::date, $6, $7, $8)
       returning id, amount, type, category, description, date, context, owner_id as "ownerId", created_at as "createdAt"`,
      [
        input.amount,
        input.type,
        input.category,
  ...
  async listBudgets(userId: string, context: FinanceContext) {
    const result = await db.query(
      `select id, name, category_id as "categoryId", amount, spent, context
       from budgets
       where user_id = $1 and context = $2
       order by name asc`,
      [userId, context],
  ...
  async createBudget(input: {
    ...
    const result = await db.query(
      `insert into budgets (name, category_id, amount, spent, context, user_id)
       values ($1, $2, $3, 0, $4, $5)
       returning id, name, category_id as "categoryId", amount, spent, context`,
      [input.name, input.categoryId, input.amount, input.context, input.userId],
    );
    const row = result.rows[0]!;
    return {
      ...row,
  ...
  async listHouseholdMembers(userId: string) {
    const result = await db.query(
      `select cm.id as "memberId", cm.display_name as "memberName", cm.contribution_amount as amount, c.id as "householdId", c.name as "householdName"
