...
async function ensureContextId(userId: string): Promise<string> {
  ...
  const created = await db.query<{ id: string }>(
    `
      ...
      VALUES ($1, 'Películas agregadas manualmente')
      RETURNING id
    `,
    [userId],
  );
...
export const moviesRepository = {
  async listMoviesForUser(userId: string) {
    const sql = `
      SELECT
        rs.id,
        rs.title,
    ...
    const result = await db.query<DbMovie>(sql, [userId]);
    return result.rows;
  },

  async createManualMovie(userId: string, input: UpsertMovieInput): Promise<DbMovie> {
    const contextId = await ensureContextId(userId);
    const result = await db.query<DbMovie>(
      `
        ...
        VALUES ($1, $2, 'activity', $3, $4, true, $5::timestamptz)
        RETURNING
          id,
          title,
    ...
    const result = await db.query<DbMovie>(
      `
        ...
        RETURNING
          ...
          (SELECT name FROM profiles WHERE id = user_id) AS owner_name
      `,
      [movieId, userId, input.title, input.description, input.startAt],
    );
