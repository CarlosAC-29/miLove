...
interface UpsertRestaurantInput {
  title: string;
  description: string;
  startAt: string;
}
...
async function ensureContextId(userId: string): Promise<string> {
  ...
  const created = await db.query<{ id: string }>(
    `
      ...
      VALUES ($1, 'Restaurantes agregados manualmente')
      RETURNING id
    `,
    [userId],
  );
...
export const restaurantsRepository = {
  async listRestaurantsForUser(userId: string) {
    const sql = `
      SELECT
        rs.id,
        rs.title,
    ...
    const result = await db.query<DbRestaurant>(sql, [userId]);
    return result.rows;
  },

  async createManualRestaurant(userId: string, input: UpsertRestaurantInput): Promise<DbRestaurant> {
    const contextId = await ensureContextId(userId);
    const result = await db.query<DbRestaurant>(
      `
        ...
        VALUES ($1, $2, 'restaurant', $3, $4, true, $5::timestamptz)
        RETURNING
          id,
          title,
    ...
    const result = await db.query<DbRestaurant>(
      `
        ...
        RETURNING
          ...
          (SELECT name FROM profiles WHERE id = user_id) AS owner_name
      `,
      [restaurantId, userId, input.title, input.description, input.startAt],
    );

    return result.rows[0] ?? null;
  },

  async deleteRestaurant(userId: string, restaurantId: string): Promise<boolean> {
